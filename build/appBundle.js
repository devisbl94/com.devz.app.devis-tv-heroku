...
var APP_com_devz_app_devis_tv = (function () {
  ...
  class FocusManager extends lng.Component {
    ...
    _firstFocusableIndex() {
      ...
      const firstItem = this.items.reduce((acc, item, idx) => {
        if (!item.skipFocus) {
          acc.push({ ...item,
            originalIdx: idx
          });
    ...
    _lastFocusableIndex() {
      if (!this.items.length) return 0;
      const lastItem = this.items.reduce((acc, item, idx) => {
        if (!item.skipFocus) {
          acc.push({ ...item,
            originalIdx: idx
          });
    ...
    selectPrevious() {
      ...
      const previousItemIndex = this.items.map(item => !!item.skipFocus).lastIndexOf(false, this._selectedIndex - 1);
    ...
    selectNext() {
      ...
      const nextIndex = this.items.findIndex((item, idx) => !item.skipFocus && idx > this._selectedIndex);
    ...
    _getIndexOfItemNear(selected, prev) {
      ...
      const prevMiddle = [itemX + prevItem.w / 2, itemY + prevItem.h / 2]; // Get all item center points from selected

      const selectedCoordArray = selected.items.map((item, index) => {
  ...
  class Column extends FocusManager {
    ...
    checkSkipPlinko(prev, next) {
      ...
      const up = direction > 0; // Grab all items below prev if up or all items before prev if down

      const prevItems = up ? this.items.slice(prevIndex).map(i => ({
      ...
      const endOfMultiSkipPlinkos = prevItems.find(i => i.skipPlinko && !this.items[i.index + direction].skipPlinko);
    ...
    _performRender() {
      this._whenEnabled.then(() => {
        if (!this.Items.children.length) {
          ...
          let scrollItem = this.selectedIndex > this._lastScrollIndex ? this.Items.children[this._lastScrollIndex - this._scrollIndex] : this.selected;
    ...
    _isOnScreen(child) {
      ...
      const {
        h
      } = child; // check that the child is inside the bounds of the stage

      const withinTopStageBounds = ItemY + h > 0; // stage height needs to be adjusted with precision since all other values assume the original height and width (pre-scaling)

      const withinBottomStageBounds = ItemY < this.stage.h / this.stage.getRenderPrecision(); // check that the child is inside the bounds of any clipping
  ...
  class Row extends FocusManager {
    ...
    _isOnScreen(child) {
      ...
      const withinLeftStageBounds = ItemX > 0; // stage width needs to be adjusted with precision since all other values assume the original height and width (pre-scaling)

      const withinRightStageBounds = ItemX + w < this.stage.w / this.stage.getRenderPrecision(); // check that the child is inside the bounds of any clipping
  ...
  class MediaItem extends lng.Component {
    static _template() {
      const textStyle = {
  ...
  class MediaCarousel extends lng.Component {
    static _template() {
      return {
        x: 100,
        h: 650,
    ...
    _init() {
      ...
      this._endpoint(this._category).then(response => {
